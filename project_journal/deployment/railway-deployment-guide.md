...
*   **Shared Library (`libs/libs`):** You do **not** need a separate build step for shared libraries like `libs/libs`. The standard NestJS/TypeScript build process automatically builds any dependent libraries when you build an application that uses them.
